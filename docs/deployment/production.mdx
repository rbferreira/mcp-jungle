...
After the first boot in enterprise mode, run `mcpjungle init-server` once from your client machine to create the initial admin identity and authenticate your CLI. Enterprise initialization requires the one-time bootstrap token:

```bash
mcpjungle init-server --bootstrap-token "$MCPJUNGLE_BOOTSTRAP_TOKEN"
```
...
1. Run the STDIO image when the gateway must launch `npx` or `uvx` servers. Use the minimal image for HTTP-only deployments.
...
<CardGroup cols={2}>
  <Card title="DigitalOcean App Platform" icon="cloud" href="/deployment/digitalocean">
    Deploy the staged Auth0 and PostgreSQL architecture.
  </Card>
